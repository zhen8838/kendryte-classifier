...
def freeze_graph(model_def, input_dir, output_graph, class_num):
    sess = tf.InteractiveSession()
    meta_file, ckpt_file = base_func.get_model_filenames(input_dir)

    network = importlib.import_module(model_def)

    images_placeholder = tf.placeholder(tf.float32, shape=(None, 224, 224, 3), name='inputs')
...
def main(args):
    freeze_graph(args.model_def, args.ckpt_dir, args.output_file, args.class_num)
